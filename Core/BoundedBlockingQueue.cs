...
namespace Timok.Core {
	...
	/// current object.  
	/// 
	/// Producer thread(s) will block on Enqueue operations until another
	...
	/// 
	/// No user code is needed to handle this "ping-pong" between locking/unlocking consumers and producers. 
	/// </summary>
	public sealed class BoundedBlockingQueue : ICollection {
		...
		#region Constructors
		/// <summary>
		/// Create instance of Queue with Bounded number of elements. After that many elements are used, 
		/// another Enqueue operation will "block" or wait until a Consumer calls Dequeue to free a slot.  
		/// Likewise, if the queue is empty, a call to Dequeue will block until another thread calls Enqueue.
		/// </summary>
		...
		#region Properties
		/// <summary>
		/// Gets the object values currently in the queue. If queue is empty, this will return a zero length 
		/// array. The returned array length can be 0 to Size. This method does not modify the queue,
		/// but returns a shallow copy of the queue buffer containing the objects contained in the queue.
		/// </summary>
		public object[] Values {
			get {
				// Copy used elements to a new array of "count" size.  Note a simple Buffer copy will not work as 
				//head could be anywhere and we want a zero based array.
				object[] values;
				lock(syncRoot) {
		...
		/// <summary>
		/// Adds an object to the end of the queue. If queue is full, this method will block until another 
		/// thread calls one of the Dequeue methods.  This method will wait "Timeout.Infinite" until queue 
		/// has a free slot.
		/// </summary>
		...
		/// <summary>
		/// Adds an object to the end of the queue. If queue is full, this method will block until another 
		/// thread calls one of the Dequeue methods or millisecondsTimeout expires.  If timeout, method will 
		...
		/// If Enqueue is successfull, this will return true; otherwise false if queue is full.
		/// </summary>
		/// <param name="value"></param>
		...
		/// Removes and returns the object at the beginning of the Queue.
		/// If queue is empty, method will block until another thread calls one of
		/// the Enqueue methods or millisecondsTimeout expires.	If timeout, method will throw 
		...
		/// Will return false if queue is empty and set value to null, otherwise will return true
		...
		/// <summary>
		/// GetEnumerator not implemented. You can't enumerate the active queue as you would an array as it is 
		/// dynamic with active gets and puts. You could if you locked it first and unlocked after
		/// enumeration, but that does not work well for GetEnumerator. The recommended method is to Get Values
		/// and enumerate the returned array copy. That way the queue is locked for only a short time and a copy returned
		/// so that can be safely enumerated using the array's enumerator. You could also create a custom 
		/// enumerator that would dequeue the objects until empty queue,
