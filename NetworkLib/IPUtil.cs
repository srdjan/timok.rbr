...
namespace Timok.NetworkLib {
	public static class IPUtil {
		const string PV4_REG_EXPR_PATTERN = @"\A(25[0-5]|2[0-4]\d|[0-1]?\d?\d)(\.(25[0-5]|2[0-4]\d|[0-1]?\d?\d)){3}\z"; //IPv4 
		...
		public static int ToInt32(string pIPAddressStr) {
			...
			try {
				...
				if (pIPAddressStr == null || pIPAddressStr.Length < 7 || pIPAddressStr.Length > 15) {
					throw new Exception(string.Format("IPUtil.ToInt32: Invalid IPAddress: {0}", pIPAddressStr));
				}
				...
				throw new Exception(string.Format("IPUtil.ToInt32: IPAddress={0}, Exception:\r\n{1}", pIPAddressStr, _ex), _ex.InnerException);
			}
		...
		public static string ToString(int pIPAddressInt) {
			...
			try {
				...
				throw new Exception(string.Format("IPUtil.ToString: IPAddress={0}, Exception:\r\n{1}", pIPAddressInt, _ex), _ex.InnerException);
			}
