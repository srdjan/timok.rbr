...
SET exportfile=rbrdb-%dt%_%tm%.7z

7z a -r "c:\temp\%exportfile%" "c:\temp\dbbackup\267\*.*"
copy "c:\temp\%exportfile%" "c:\temp\dbbackup\%exportfile%"


REM --------------- Ftp -----------------------------------------
cd c:\temp

if exist PutFiles.ftp del PutFiles.ftp
echo off

echo srdjan>> PutFiles.ftp
echo 0gster>> PutFiles.ftp
echo binary >> PutFiles.ftp
echo put %exportfile% >> PutFiles.ftp
echo quit >> PutFiles.ftp 

echo on 

FTP -s:PutFiles.ftp 201.30.72.134
FTP -s:PutFiles.ftp 201.30.72.135
FTP -s:PutFiles.ftp 201.30.72.137
FTP -s:PutFiles.ftp 201.30.72.136
